...
import { LayoutDashboard, BookOpen, PartyPopper, LogOut, Menu, Sun, Moon, ChevronsUpDown } from "lucide-react";
...
const nav: NavItem[] = [
  ...
  { href: "/teacher/clubs", label: "Clubs", icon: PartyPopper },
];
    ...
    <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
      {nav.map((item) => {
        ...
        return (
          ...
          >
            <Icon className={`w-4.5 h-4.5 shrink-0 ${active ? "" : "text-sidebar-foreground/70"}`} />
...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    fetch("/api/teacher/me")
      .then((r) => r.json())
      .then((d) => {
        if (d.teacher?.fullName) setTeacherName(d.teacher.fullName);
        if (d.teacher?.avatarUrl) setAvatarUrl(d.teacher.avatarUrl);
      })
      .catch(() => {});
  }, []);
  ...
  const activeLabel = nav.find((n) => isActive(n.href))?.label ?? "Teacher Portal";
      ...
      <aside className="hidden lg:flex w-64 shrink-0 flex-col bg-sidebar border-r border-sidebar-border">
        <div className="flex items-center gap-2.5 px-5 py-5 border-b border-sidebar-border">
          ...
          <div className="overflow-hidden">
            <p className="text-sidebar-foreground text-sm font-semibold truncate">{teacherName}</p>
            <p className="text-sidebar-foreground/50 text-xs">Teacher Portal</p>
          </div>
        </div>
